      ...
      <main className="px-3 py-8 sm:py-10">
        <Container>
          <div className="grid gap-6 lg:grid-cols-[1.1fr_0.9fr]">
            <div>
              ...
              {cart.length === 0 ? (
                ...
                <div className="mt-8 grid gap-4">
                  {cart.map((item) => (
                    <Card key={item.productId} className="flex flex-col gap-5 p-5 sm:flex-row sm:items-center sm:justify-between">
                      <div className="flex gap-4">
                        <img src={item.image || 'https://placehold.co/320x320/FFE7CC/1F2937?text=Mafalda'} alt={item.title} className="h-24 w-24 rounded-[24px] object-cover" />
                        <div>
                          ...
                          <button type="button" onClick={() => removeFromCart(item.productId)} className="mt-3 text-sm font-semibold text-coral">
                            Quitar producto
                          </button>
                        </div>
                      </div>
                      ...
                      <div className="flex flex-wrap items-center gap-4 sm:justify-end">
                        <div className="flex items-center rounded-full border border-stone-200 bg-white px-3 py-2">
                          <button type="button" className="h-10 w-10 rounded-full bg-[#fff4df] text-lg" onClick={() => updateQty(item.productId, item.qty - 1)}>-</button>
                          <span className="min-w-12 px-4 text-center font-semibold">{item.qty}</span>
                          <button type="button" className="h-10 w-10 rounded-full bg-[#fff4df] text-lg" onClick={() => updateQty(item.productId, item.qty + 1)}>+</button>
                        </div>
            ...
            <div>
              <Card className="sticky top-28 bg-ink p-7 text-white">
                ...
                <div className="mt-6 grid gap-4 rounded-[28px] bg-white/10 p-5">
